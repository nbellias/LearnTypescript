...
const numEmitter = (0, rxjs_1.of)(1, 2, 3, 4);
const modulus = numEmitter.pipe((0, rxjs_1.map)((value) => {
    console.log(`received : ${value}`);
    return value % 2;
}));
const stringMap = numEmitter.pipe((0, rxjs_1.map)((value) => { return value * 2; }), (0, rxjs_1.map)((value) => { return `str_${value}`; }));
const emitter = (0, rxjs_1.of)(1, 2, "Nikos", { name: "test" }, new Date());
const emitterPipe = emitter.pipe((0, rxjs_1.map)((value) => {
    console.log(`value: ${value instanceof Object ? JSON.stringify(value) : value % 2}`);
    return value;
}));
const emitArray = (0, rxjs_1.from)(["Margarita", { id: 12345 }, new Date(), 544]);
// Universities API
const API_URL = 'http://universities.hipolabs.com/search?country=' + 'Malta';
const emitUniversities = (api) => {
    return (0, rxjs_1.from)(axios_1.default.get(api));
};
const emitUniversitiesPiped = emitUniversities(API_URL).pipe((0, rxjs_1.map)((value) => {
